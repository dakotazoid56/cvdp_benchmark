...
class vLLM_Instance:
    """
    Example implementation of a vLLM model instance.

    This provides the same interface as OpenAI_Instance but uses vLLM's API.
    """

    def __init__(self, context: Any = "You are a helpful assistant.", key: Optional[str] = None, model: str = "/gpfs/scratch/bsc70/hpai/storage/projects/heka/models/Qwen3-Coder-30B-A3B-Instruct"):
        """
        Initialize a vLLM model instance.

        Args:
            context: The system prompt or context for the model
            key: should be empty for local models
            model: The vLLM model version to use
        """
        ...
        logging.info(f"Created vLLM Model using the provided key. Using model: {self.model}")
    ...
    def prompt(self, prompt: str, schema: Optional[str] = None, prompt_log: str = "", 
        ...
        # This is where you would call the actual vLLM API
        # Uncomment and update when implementing with actual vLLM API
